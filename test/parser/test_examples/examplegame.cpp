...
void Partialchess::wshortcastling() {
	Player *current = getCurrentPlayer();
	if (current->getColour() == WHITE) {
		bool cond = true;
		cond = cond && ((trackers[WSC]==1,trackers[BSC]==1));
		cond = cond && ((game->getBoard()->getSquare(0,5)).isEmpty());
		cond = cond && ((game->getBoard()->getSquare(0,6)).isEmpty());
		cond = cond && (squaresControlled(getOppositePlayer()).count((game->getBoard()->getSquare(0,5))) == 0);
		cond = cond && (squaresControlled(getOppositePlayer()).count((game->getBoard()->getSquare(0,6))) == 0);
		if (cond) {
			GameMove move;
			move.piece = game->getBoard()->getSquare(0,4).getPiece();
			move.square = game->getBoard()->getSquare(0,6);
			move.instr = wshortcastling;
			moves.push_back(move);
			GameMove *parent = &move;
			GameMove *aux0 = new GameMove;
			aux0->piece = game->getBoard()->getSquare(0,7).getPiece();
			aux0->square = game->getBoard()->getSquare(0,5);
...
void Partialchess::wlongcastling() {
	Player *current = getCurrentPlayer();
	if (current->getColour() == WHITE) {
		bool cond = true;
		cond = cond && ((trackers[WLC]==1));
		cond = cond && ((game->getBoard()->getSquare(0,2)).isEmpty());
		cond = cond && ((game->getBoard()->getSquare(0,3)).isEmpty());
		cond = cond && (squaresControlled(getOppositePlayer()).count((game->getBoard()->getSquare(0,2))) == 0);
		cond = cond && (squaresControlled(getOppositePlayer()).count((game->getBoard()->getSquare(0,3))) == 0);
		if (cond) {
			GameMove move;
			move.piece = game->getBoard()->getSquare(0,4).getPiece();
			move.square = game->getBoard()->getSquare(0,2);
			move.instr = wlongcastling;
			moves.push_back(move);
			GameMove *parent = &move;
			GameMove *aux0 = new GameMove;
			aux0->piece = game->getBoard()->getSquare(0,0).getPiece();
			aux0->square = game->getBoard()->getSquare(0,3);
			parent->aux->reset(aux0));
			parent = aux0;
		}
	}
...
void Partialchess::wpawntwosquares() {
	Player *current = getCurrentPlayer();
	if (current->getColour() == WHITE) {
		Piece *piece = NULL;
		for (vector <Piece *> :: iterator it = current.getPieces().begin(); it != current.getPieces().end(); it ++) {
			...
			if (piece.getName() == pawn) {
				...
				cond = cond && ((game->getBoard()->getSquare($2,piece->getSquare()->getCol())).isEmpty());
				cond = cond && ((game->getBoard()->getSquare($3,piece->getSquare()->getCol())).isEmpty());
				if (cond) {
					GameMove move;
					move.piece = game->getBoard()->getSquare($1,piece->getSquare()->getCol()).getPiece();
					move.square = game->getBoard()->getSquare($3,piece->getSquare()->getCol());
		...
		for (vector <Piece *> :: iterator it = current.getPieces().begin(); it != current.getPieces().end(); it ++) {
			...
			if (piece.getName() == pawn) {
				...
				if (cond) {
					...
					move.square = game->getBoard()->getSquare(5,trackers[EP_COL]);
					move.instr = wenpassant;
					moves.push_back(move);
					GameMove *parent = &move;
					GameMove *aux0 = new GameMove;
					aux0->piece = NULL;
					aux0->square = game->getBoard()->getSquare(4,trackers[EP_COL]);
					parent->aux->reset(aux0));
					parent = aux0;
				}
			}
...
void PartialchessGame::promotion() {
	...
	if (getCurrentPlayer().getColour == WHITE) {
		...
		if (cond) {
			GameMove promotion0 = GameMove(move);
			promotion0.instr = "#TARGET(queen)";
			temp.push_back(promotion);
			GameMove promotion1 = GameMove(move);
			promotion1.instr = "#TARGET(rook)";
			temp.push_back(promotion);
			GameMove promotion2 = GameMove(move);
			promotion2.instr = "#TARGET(bishop)";
			temp.push_back(promotion);
			GameMove promotion3 = GameMove(move);
			promotion3.instr = "#TARGET(knight)";
			temp.push_back(promotion);
			moves.erase(it);
			it--;
		}
...
void PartialchessGame::preMove(GameMove& move) {
	...
	if (cond) {
		#PREMOVE{;
		#COND(move.piece->getSquare()->getRow()==0);
		#COND(move.piece->getSquare()->getCol()==4);
		trackers[WSC]=0;
		trackers[WLC]=0;
		};
	}
...
void PartialchessGame::postMove(GameMove& move) {
	...
	if (cond) {
		#POSTMOVE{;
		#COND(getRepetition>trackers[REP]);
